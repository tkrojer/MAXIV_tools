...
def validate_cif(modelFilePath, sfFilePath):
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv, "m:s:h", ["model=", "sf=", "help"])
    except getopt.GetoptError:
        sys.exit(2)

    for opt, arg in opts:
        if opt == '-h':
            print("pdb_validate_cif.py -m model.cif -s sf.cif")
            sys.exit(2)
        elif opt in ("-m", "--model"):
    ...
    if os.path.isfile(modelFilePath) and os.path.isfile(sfFilePath):
        validate_cif(modelFilePath, sfFilePath)
    else:
        print("pdb_validate_cif.py -m model.cif -s sf.cif")
